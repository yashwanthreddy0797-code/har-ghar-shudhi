#!/usr/bin/env python3
"""
Optimize Ashwagandha hero scroll frames for the web.

Aligns each frame by the central bottle region so the product stays locked
during scroll playback.

Usage:
  python3 scripts/optimize-ashwagandha-sequence.py
  python3 scripts/optimize-ashwagandha-sequence.py --source "/path/to/frames"
"""
...
DEFAULT_SOURCE = ROOT / "public" / "hero" / "ashwagandha" / "source"
OUT = ROOT / "public" / "hero" / "ashwagandha" / "sequence"
MANIFEST = ROOT / "lib" / "hero/ashwagandha-sequence.manifest.json"

# Frames with baked-in background / visual glitches
EXCLUDED_FRAME_INDICES: list[int] = [20, 32, 33, 46, 62, 89, 98]
...
def compute_uniform_scale(sources: list[Path]) -> float:
    bottle_heights: list[float] = []
    for src in sources:
        anchor = bottle_anchor(Image.open(src).convert("RGBA"))
        if anchor:
            bottle_heights.append(anchor[2])

    if not bottle_heights:
        return OUTPUT_H / 1280

    median_h = statistics.median(bottle_heights)
    return TARGET_STRIP_HEIGHT / median_h


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Optimize Ashwagandha hero scroll frames"
    )
    parser.add_argument(
        "--source",
        type=Path,
    ...
    print(
        f"Optimizing {len(sources)} frames — bottle anchor "
    ...
    manifest = {
        ...
        "height": OUTPUT_H,
        "format": "webp",
        "pathPrefix": "/hero/ashwagandha/sequence/frame-",
